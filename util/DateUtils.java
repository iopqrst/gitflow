...
public class DateUtils {
	...
	/**
	 * 截止日期（当前时间+cycle(天)） 如果今天下午2点购买产品，产品期限为一天，则用户可以在 后天凌晨0点钱可以是使用该产品
	 * 
	 * 如果是不限制天数，则返回null
	 */
	public static Date getExpiresTime(int cycle) {
		Integer unlimte = Integer.parseInt(SystemConfig
				.getString("unlimit_tag"));
		if (unlimte.intValue() == cycle) {
			return null;
		}

		Calendar calendar = Calendar.getInstance();
		calendar.add(Calendar.DAY_OF_YEAR, (cycle + 1));

		Date date = parseDate(formatDate(DATE_PATTERN, calendar.getTime()),
				DATE_PATTERN);
